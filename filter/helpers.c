...
{
    //image[i][j] make RGBTRIPLE grey(calculate the avg of the values of RGB  in that pixel)
    for(int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            float rgbGray = round(image[j][i].rgbtRed + image[j][i].rgbtGreen + image[j][i].rgbtBlue)/3;
...
{
    int sepiaRed, sepiaGreen = 0, sepiaBlue;
    for(int i = 0; i < width; i++)
    {
        for(int j = 0; j < height; j++)
        {
